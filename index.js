...
function startGame(gameID){
    ...
    currentFolderID = -1;
    currentProgress = 0;
    if(gameID == 7 || gameID == 8 || gameID == 9) { maxForBar = 4; tree = createTree(8); }
    else if(gameID == 4 || gameID == 5 || gameID == 6) { maxForBar = 3; tree = createTree(7); }
...
function startSpeedrun(time){
    timeLeft = time;
    setInterval(tickDownTimer, 1000)
    if(timeLeft <= 0){
        gameOver();
    }
}
